...
import { modelById } from '../../../../lib/models/index.js';
export default function Page({ params }){
  ...
  return <div className="wrap" style={{ maxWidth:1600 }}>
    <div className="crumb"><a href="/explore#reference">Explore</a><span>/</span><a href="/reference/diagrams">Diagrams</a><span>/</span><span style={{ color:'var(--ink)' }}>{def.title}</span></div>
    ...
    <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginTop:14, alignItems:'center' }}>{def.books.map(b=>{ const bk=BOOKS.find(x=>x.name===b); return bk?<a key={b} className="chip" href={`/book/${bk.slug}`}>{b}</a>:null; })}{modelById(def.id) && <a className="btn sm" href={`/reference/models/${def.id}`}>See it in 3D</a>}</div>
