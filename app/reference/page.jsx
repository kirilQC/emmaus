...
export default function Page(){
  return <div className="wrap">
    ...
    <div className="grid2" style={{ marginTop:40 }}>
      {SHELVES.map(([t,h,i,c,d])=><a key={t} className="card" href={h} style={{ padding:'24px 26px', color:'var(--ink)', display:'flex', gap:18 }}><span className="gi" dangerouslySetInnerHTML={{ __html:ico(i,24) }} /><span><span className="lab">{c}</span><span style={{ display:'block', fontSize:28, fontWeight:500, lineHeight:1.1, marginTop:4, fontFamily:'var(--f-disp)' }}>{t}</span><span style={{ color:'var(--dim)', fontSize:16, display:'block', marginTop:6 }}>{d}</span></span></a>)}
    </div>
  </div>;
}
