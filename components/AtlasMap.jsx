  ...
  useEffect(()=>{
    ...
    import('maplibre-gl').then(({ default: maplibregl })=>{
      ...
      map.on('load',()=>{
        ...
        map.addLayer({ id:'regions-fill', type:'fill', source:'regions', paint:{ 'fill-color':['get','color'], 'fill-opacity':0.16 } });
        map.addLayer({ id:'regions-line', type:'line', source:'regions', paint:{ 'line-color':['get','color'], 'line-width':1.2, 'line-opacity':0.7, 'line-dasharray':[3,2] } });
        ...
        map.addLayer({ id:'region-labels', type:'symbol', source:'region-labels', layout:{ 'text-field':['get','name'], 'text-font':FONTB, 'text-size':['interpolate',['linear'],['zoom'],4,10,8,14], 'text-letter-spacing':0.18, 'text-max-width':8 }, paint:{ 'text-color':['get','color'], 'text-halo-color':'#171510', 'text-halo-width':1.4, 'text-opacity':0.9 } });
        // Routes
        def.routes.forEach(r=>{
          const feats=r.legs.map((l,i)=>({ type:'Feature', properties:{ leg:i, label:l.label, dashed:!!(l.dashed||r.dashed) }, geometry:{ type:'LineString', coordinates:l.stops.map(s=>PLACES[s].c) } }));
        ...
        map.addLayer({ id:'place-labels', type:'symbol', source:'places', layout:{ 'text-field':['get','name'], 'text-font':FONT, 'text-size':['interpolate',['linear'],['zoom'],5,10,9,13], 'text-offset':[0.8,0], 'text-anchor':'left', 'text-optional':true }, paint:{ 'text-color':'#ece7dc', 'text-halo-color':'#171510', 'text-halo-width':1.4 } });
  ...
  return <div className="atlas">
    ...
    <div className="atlas-side">
      {def.routes.length>0 && <div className="card" style={{ padding:'14px 16px' }}><div className="lab" style={{ marginBottom:8 }}>Routes</div>
        {def.routes.map(r=><label key={r.id} className="atlas-route"><input type="checkbox" checked={!!visible[r.id]} onChange={e=>setVisible(v=>({ ...v, [r.id]:e.target.checked }))} /><span className="swatch" style={{ background:r.color, borderStyle:r.dashed?'dashed':'solid' }} /><span>{r.label}</span></label>)}
      </div>}
