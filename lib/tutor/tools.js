...
export function grounding(ctx){
  return { type:'context', calls:ctx.calls, passages:ctx.passages.map(p=>({ label:p.label, href:`/book/${p.book}/${p.ch}` })), hits:ctx.hits.slice(0,8).map(h=>({ label:h.ref, href:`/book/${h.book}/${h.ch}` })), xrefs:ctx.xrefs.slice(0,8).map(x=>({ label:x.ref, href:`/book/${x.book}/${x.ch}` })), notes:ctx.notes.length?`Tyndale Open Study Notes on ${[...new Set(ctx.notes.map(n=>n.ref.replace(/:\d+$/,'')))].join(', ')}`:'', intro:ctx.intro?.text?`Tyndale introduction to ${ctx.intro.book}`:'', people:ctx.people.map(p=>p.name), places:ctx.places.map(p=>p.name), studies:ctx.studies.map(s=>({ label:s.title, href:s.path, evidence:s.evidence })), pages:ctx.pages.slice(0,6).map(p=>({ label:p.title, href:p.path, kind:p.kind })) };
}
