...
export function wire(p,ctx){const $=s=>document.querySelector(s);const $$=s=>[...document.querySelectorAll(s)];const render=ctx.rerender;
 ...
 if(p[0]==='book'&&p[2]){const s=p[1],n=+p[2];const book=bySlug(s);if(book&&Number.isInteger(n)&&n>=1&&n<=book.ch){
   // Hydrate saved notes before recording the visit, so a fresh visit cannot mask a cloud backup.
   S.LAST={book:s,ch:n};
   Promise.resolve(S._syncReady).then(()=>{if(location.pathname===`/book/${s}/${n}`){S.LAST={book:s,ch:n};S.save()}});
  }const sn=$('#savenote');if(sn)sn.onclick=()=>{S.NOTES[s+':'+n]=$('#note').value;S.save();$('#notestat').textContent='Saved'};
  const ag=$('#askgo');if(ag)ag.onclick=e=>{e.preventDefault();const q=$('#askq').value.trim();ctx.navigate('/tutor'+(q?'?q='+encodeURIComponent(q):''))};const aq=$('#askq');if(aq)aq.onkeydown=e=>{if(e.key==='Enter')ag.click()}}
